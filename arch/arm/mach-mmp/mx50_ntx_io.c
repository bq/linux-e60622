...
#if 0
#define GPIO_LED_ON		(5*32 + 26)	/*GPIO_6_26 */
#define GPIO_ACT_ON		(5*32 + 24)	/*GPIO_6_24 */
#define GPIO_CHG_LED	(5*32 + 25)	/*GPIO_6_25 */
#else
...
static int g_ioctl_SD_status, g_ioctl_USB_status, g_ioctl_rotary_status;
...
//						printk ("[%s-%d] battery ===> %d\n", __func__, __LINE__,i);
		...
		case CM_AUDIO_PWR:
	        if (p) {
//	        	gpio_direction_output (MFP_PIN_GPIO46,0);		// Turn on audio power
//	        	gpio_activate_audio_ports(); // 會造成IO REQUEST ERROR !?
	        }
	        else {
//	        	gpio_direction_input(MFP_PIN_GPIO46);	// turn off audio power
...
//		 743,  767,  791,  812,  835,  860,  885,  909,  935,  960,  985, 1010, 1023,
		 767,  791,  812,  833,  852,  877,  903,  928,  950,  979,  903, 1019, 1023,
	};
...
#if 0
	/* ON_LED */
	mxc_iomux_v3_setup_pad(MX50_PAD_OWIRE__GPIO_6_26);
