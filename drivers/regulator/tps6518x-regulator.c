 ...
 */
enum {
    REG_TPS6518X_TMST_VALUE = 0x0,
    REG_TPS6518X_ENABLE,
    REG_TPS6518X_VP_ADJUST,
    REG_TPS6518X_VN_ADJUST,
    REG_TPS6518X_VCOM_ADJUST,
    REG_TPS6518X_INT_ENABLE1, /* 5 */
    REG_TPS6518X_INT_ENABLE2,
    REG_TPS6518X_INT_STATUS1,
    REG_TPS6518X_INT_STATUS2,
    REG_TPS6518X_PWR_SEQ0,
    REG_TPS6518X_PWR_SEQ1, /* 10 */
    REG_TPS6518X_PWR_SEQ2,
    REG_TPS6518X_TMST_CONFIG,
    REG_TPS6518X_TMST_OS,
    REG_TPS6518X_TMST_HYST,
    REG_TPS6518X_PG_STATUS, /* 15 */
    REG_TPS6518X_REVID,
    REG_TPS6518X_FIX_READ_POINTER,
};
#define TPS6518X_REG_NUM        18
#define TPS6518X_MAX_REGISTER   0xFF

/*
 * TPS65185 registers that are redefined differently
 */
#define REG_TPS65185_VADJ	REG_TPS6518X_VP_ADJUST
#define REG_TPS65185_VCOM1	REG_TPS6518X_VN_ADJUST
#define REG_TPS65185_VCOM2	REG_TPS6518X_VCOM_ADJUST
#define REG_TPS65185_UPSEQ0	REG_TPS6518X_PWR_SEQ0
#define REG_TPS65185_UPSEQ1	REG_TPS6518X_PWR_SEQ1
#define REG_TPS65185_DWNSEQ0	REG_TPS6518X_PWR_SEQ2
#define REG_TPS65185_DWNSEQ1	REG_TPS6518X_TMST_CONFIG
#define REG_TPS65185_TMST1	REG_TPS6518X_TMST_OS
#define REG_TPS65185_TMST2	REG_TPS6518X_TMST_HYST
/* 2011/2/3 FY11 : Supported the function to save VCOM. */
#define REG_TPS65185_INT_STATUS1	REG_TPS6518X_INT_STATUS1
/* 2011/05/25 FY11 : Changed the flag to confirm completion of reading temperature. */
#define REG_TPS6518x_INT_STATUS2	REG_TPS6518X_INT_STATUS2
 ...
 */
/* 2011/5/26 FY11 : Modified to use absolute value for VCOM (TPS6518X/1). */
#define TPS6518X_VCOM_MIN_uV	      0
#define TPS6518X_VCOM_MAX_uV	2750000 
#define TPS6518X_VCOM_MIN_SET         0
#define TPS6518X_VCOM_MAX_SET       255
/* 2011/05/31 FY11 : Fixed invalid step uV. */
#define TPS6518X_VCOM_BASE_uV     10784
#define TPS6518X_VCOM_STEP_uV     10784
 ...
 */
enum {
	/* as specific event in TPS6518X */
	TPS6518X_TP_VEE_PWRUP,
	TPS6518X_TP_VNEG_PWRUP,
	TPS6518X_TP_VPOS_PWRUP,
	TPS6518X_TP_VDDH_PWRUP,
	TPS6518X_TP_VDDH_PWRDN,
	TPS6518X_TP_VPOS_PWRDN,
	TPS6518X_TP_VNEG_PWRDN,
	TPS6518X_TP_VEE_PWRDN,
};
#define TPS6518X_TP_COUNT 8

struct tps6518x_timings {
...
#define TPS6518X_DEFAULT_POWSEQ 0xE4
...
static struct tps6518x_timings power_sequence_timing = {
	.vddh_seq = 3,
	.vpos_seq = 2,
	.vee_seq = 1,
	.vneg_seq = 0,
 ...
 */
static int tps6518x_register_regulator(struct tps6518x *tps6518x, int reg,
			      struct regulator_init_data *initdata);

/*
 * TPS6518X device initialization and exit.
 */
int tps6518x_device_init(struct tps6518x *tps6518x, int irq,
		       struct tps6518x_platform_data *pdata);
void tps6518x_device_exit(struct tps6518x *tps6518x);
...
/*
 * TPS6518X Timing Parameters
 */
int tps6518x_set_timings(struct regulator_dev *reg, const struct tps6518x_timings *ts);
...
static int tps6518x_v3p3_enable(struct regulator_dev *reg)
{
	...
	struct i2c_client *client = tps6518x->i2c_client;
...
/* 2011/05/26 FY11 : Supported TPS65181. */
	switch (TPS6518x_REVID_VERSION(tps6518x->revId))
	{
		...
		case 5 : /* TPS65185 */
		case 6 : /* TPS65186 */
			ret = i2c_smbus_read_byte_data(client, REG_TPS6518X_ENABLE);
			if ( ret < 0 )
			{
			...
			ret = i2c_smbus_write_byte_data(client, REG_TPS6518X_ENABLE, new_reg_val);
			break;
		default :
			ret = -EINVAL;
			break;
...
static int set_power_sequence(struct tps6518x *tps6518x,
                              const struct tps6518x_timings *ts)
{
	struct i2c_client *client = tps6518x->i2c_client;
	...
	switch (TPS6518x_REVID_VERSION(tps6518x->revId))
	{
	   ...
	   case 1 : /* TPS65181 */
		...
		retval = i2c_smbus_write_byte_data(client, REG_TPS6518X_PWR_SEQ0, reg_val);
		break;
	   case 5 : /* TPS65185 */
		/*
...
static int set_power_delays(struct tps6518x *tps6518x,
                            const struct tps6518x_timings *ts)
{
...
/* 2011/05/26 FY11 : Supported TPS65181 to handle error. */
		...
		ret = i2c_smbus_write_byte_data(client, REG_TPS6518X_PWR_SEQ1, reg_val);
		if ( ret < 0 )
		{
			return ret;
		}
		...
		ret = i2c_smbus_write_byte_data(client, REG_TPS6518X_PWR_SEQ2, reg_val);
		if ( ret < 0 )
		{
			return ret;
		}
...
/* 2011/03/22 FY11 : Modified to handle error. */
	...
	ret = set_power_sequence(tps6518x, ts);
	if ( ret <  0 )
	{
	...
	ret = set_power_delays(tps6518x, ts);
	if ( ret <  0 )
	{
...
EXPORT_SYMBOL(tps6518x_set_timings);

static int vcom_rs_to_uV(unsigned int reg_setting)
{
	if (reg_setting <= TPS6518X_VCOM_MIN_SET)
		return TPS6518X_VCOM_MIN_uV;
	if (reg_setting >= TPS6518X_VCOM_MAX_SET)
		return TPS6518X_VCOM_MAX_uV;
/* 2011/5/26 FY11 : Modified to use absolute value for VCOM (TPS6518X/1). */
	return (reg_setting * TPS6518X_VCOM_STEP_uV);
}
...
{
	if (uV <= TPS6518X_VCOM_MIN_uV)
		return TPS6518X_VCOM_MIN_SET;
	if (uV >= TPS6518X_VCOM_MAX_uV)
		return TPS6518X_VCOM_MAX_SET;
/* 2011/5/26 FY11 : Modified to use absolute value for VCOM (TPS6518X/1). */
/* 2011/05/31 FY11 : Modified to calcurate more accurate val. */
	return (uV + TPS6518X_VCOM_STEP_uV/2) / TPS6518X_VCOM_STEP_uV;
}
...
/* 2011/05/26 FY11 : Supported TPS65181. */
static int tps6518x_vcom_set_voltage_to_pmic(struct regulator_dev *reg,
                                     int minuV, int uV)
{
	struct tps6518x *tps6518x = rdev_get_drvdata(reg);
	struct i2c_client *client = tps6518x->i2c_client;
	...
	switch (TPS6518x_REVID_VERSION(tps6518x->revId))
	{
	   case 0 : /* TPS6518X */
	   case 1 : /* TPS65181 */
		retval = i2c_smbus_read_byte_data(client, REG_TPS6518X_VCOM_ADJUST);
		if ( retval < 0 )
		{
			printk( KERN_ERR "%s fail to read i2c. %d\n", __func__, retval );
			break;
		...
		retval = i2c_smbus_write_byte_data(client, REG_TPS6518X_VCOM_ADJUST,
	                          new_reg_val);
		break;
	   case 5 : /* TPS65185 */
	   case 6 : /* TPS65186 */
...
static int tps6518x_vcom_get_voltage(struct regulator_dev *reg)
{
	...
	switch (TPS6518x_REVID_VERSION(tps6518x->revId))
	{
	   case 0 : /* TPS6518X */
	   case 1 : /* TPS65181 */
		ret = i2c_smbus_read_byte_data(client, REG_TPS6518X_VCOM_ADJUST);
		if ( ret < 0 )
		{
			printk( KERN_ERR "%s fail to read i2c. %d\n", __func__, ret );
			break;
...
static int tps6518x_vcom_is_enabled(struct regulator_dev *reg )
{
	struct tps6518x *tps6518x = rdev_get_drvdata(reg);
	int gpio = 0;
	gpio = gpio_get_value(tps6518x->gpio_pmic_vcom_ctrl);
	if ( gpio == 0 )
	{
		return 0;
	}
...
static int tps6518x_vcom_enable(struct regulator_dev *reg)
{
	...
	return i2c_smbus_write_byte_data(client, REG_TPS6518X_ENABLE, new_reg_val);
#else
...
/* 2011/1/26 FY11 : Fixed the bug cannot wait power_good. */
		if (gpio_get_value(tps6518x->gpio_pmic_pwrgood))
			return 0;
		else
			msleep(10);
...
static int tps6518x_vsys_is_enabled(struct regulator_dev *reg)
{
	struct tps6518x *tps6518x = rdev_get_drvdata(reg);
	int gpio = gpio_get_value(tps6518x->gpio_pmic_vsys);
...
static int tps6518x_vsys_enable(struct regulator_dev *reg)
{
	struct tps6518x *tps6518x = rdev_get_drvdata(reg);
	gpio_set_value(tps6518x->gpio_pmic_vsys, 1);
	printk(KERN_INFO "%s : wait VSYS_EPD.\n", __func__ );
	msleep(20);
	return 0;
}

static int tps6518x_vsys_disable(struct regulator_dev *reg)
{
	struct tps6518x *tps6518x = rdev_get_drvdata(reg);
	gpio_set_value(tps6518x->gpio_pmic_vsys, 0);
...
/* 2011/05/30 FY11 : Supported to read vcom from eMMC. */
static int tps6518x_display_first_enable = 1;
...
/* 2011/05/30 FY11 : Supported to read vcom from eMMC. */
	if(tps6518x_display_first_enable)
	{
		unsigned long ulWfIndex, ulWfAreaSize;
		unsigned long ulVcomRegVal;
/*		ulWfIndex = rawdata_index("Waveform", &ulWfAreaSize);
		...
		{
			case 1 : /* TPS65181 */
				tps6518x->vcom_uV = vcom_rs_to_uV(ulVcomRegVal);
				break;

			case 5 : /* TPS65185 */
				tps6518x->vcom_uV = vcom2_rs_to_uV(ulVcomRegVal);
				break;
		}
		tps6518x_display_first_enable = 0;
	}
...
/* 2011/1/24 FY11 : Added wakeup port control. */
	...
	if (tps6518x->pmic_setup == false) {
	   switch (TPS6518x_REVID_VERSION(tps6518x->revId))
	   {
	      case 0 : /* TPS6518X */
	      case 1 : /* TPS65181 */
#if MANUAL_VCOM_ADJ
		/* TPS6518X has VCOM_ADJ bit set by default.  If this bit is set VCOM is adjusted by I2C only, */
		cur_reg_val = i2c_smbus_read_byte_data(client, REG_TPS6518X_VN_ADJUST);
		...
		i2c_smbus_write_byte_data(client, REG_TPS6518X_VN_ADJUST, new_reg_val);
...
/*2011/05/31 FY11 : Added retry for first i2c communication with TPS65181. */
		...
		while ( retry >= 0 )
		{
			ret = i2c_smbus_read_byte_data(client, REG_TPS6518X_VN_ADJUST);
			if ( ret < 0 )
			{
				retry--;
				if ( retry < 0 )
...
/* 2011/06/13 FY11 : Modified wait time for retry after first i2c success. (1.8msec -> 10msec) */
		...
		new_reg_val = to_reg_val(cur_reg_val, fld_mask, fld_val);
		ret = i2c_smbus_write_byte_data(client, REG_TPS6518X_VN_ADJUST, new_reg_val);
		if ( ret < 0 )
		{
...
/* 2011/05/26 FY11 : Supported TPS65181. */
		ret = tps6518x_vcom_set_voltage_to_pmic(reg,tps6518x->vcom_uV,tps6518x->vcom_uV);
		if ( ret < 0 )
		{
			tps6518x_sleep(tps6518x); 
			return ret;
		...
		ret = tps6518x_set_timings(reg, &power_sequence_timing);
		if ( ret < 0 )
		{
			tps6518x_sleep(tps6518x); 
			return ret;
...
/* 2011/05/26 FY11 : Supported TPS65181. */
		/* set read therm. */
		ret = i2c_smbus_read_byte_data(client, REG_TPS6518X_TMST_CONFIG);
		if ( ret < 0 )
		{
		...
		ret = i2c_smbus_write_byte_data(client, REG_TPS6518X_TMST_CONFIG, new_reg_val);
		if ( ret < 0 )
		{
...
/* 2011/1/19 FY11 : Remove default value setting. (Use default value stored in non-volatile memory.) */
		/* set the vcom to default value */
//		tps6518x_vcom_set_voltage(reg,tps6518x->vcom_uV,tps6518x->vcom_uV);
		/* set the power sequencing */
/* 2011/03/22 FY11 : Modified to handle error. */
		while ( retry > 0 )
		{
	   ...
	   /* setup is complete */
	   tps6518x->pmic_setup = true;
	}
#if SWITCH_3V3
	/* turn on 3p3 rail */
	cur_reg_val = i2c_smbus_read_byte_data(client, REG_TPS6518X_ENABLE);
	fld_mask = BITFMASK(V3P3_SW_EN);
	fld_val = BITFVAL(V3P3_SW_EN, true);
	new_reg_val = to_reg_val(cur_reg_val, fld_mask, fld_val);
	i2c_smbus_write_byte_data(client, REG_TPS6518X_ENABLE, new_reg_val);
...
static int tps6518x_Temp_enable(struct regulator_dev *reg)
{

	return 0;
}

static int tps6518x_Temp_disable(struct regulator_dev *reg)
{

	return 0;
}

static int tps6518x_Temp_get_temperature(struct regulator_dev *reg)
{
	struct tps6518x *tps6518x = rdev_get_drvdata(reg);
...
/* 2011/05/26 FY11 : Supported TPS65181. */
	...
	switch (TPS6518x_REVID_VERSION(tps6518x->revId))
	{
		...
		case 1 : /* TPS65181 */
			...
			ret = tps6518x_wait_power_good(tps6518x);
			if ( ret < 0 )
			{
		...
		case 5 : /* TPS65185 */
		case 6 : /* TPS65186 */
			gpio_set_value(tps6518x->gpio_pmic_pwr0_ctrl, 1);
/* 2011/1/24 FY11 : Added wait for power good. */
			ret = tps6518x_wait_power_good(tps6518x);
			if ( ret < 0 )
			{
				printk( KERN_ERR "%s wait power good failed %d\n", __func__, ret );
				gpio_set_value(tps6518x->gpio_pmic_pwr0_ctrl, 0);
			}
			break;
		default :
			ret = -EINVAL;
...
static int epdc_pwr2_enable(struct regulator_dev *reg)
{
	...
	gpio_set_value(tps6518x->gpio_pmic_pwr2_ctrl, 0);
...
static int epdc_pwr2_disable(struct regulator_dev *reg)
{
	...
	gpio_set_value(tps6518x->gpio_pmic_pwr2_ctrl, 1);
...
static struct regulator_ops tps6518x_Temp_ops = {
	.enable = tps6518x_Temp_enable,
	.disable = tps6518x_Temp_disable,
	.get_voltage = tps6518x_Temp_get_temperature,
};
...
static struct regulator_ops tps6518x_vsys_ops = {
	.enable = tps6518x_vsys_enable,
	.disable = tps6518x_vsys_disable,
	.is_enabled = tps6518x_vsys_is_enabled,
};
...
i2c_retry:
	...
	tps6518x->revId = i2c_smbus_read_byte_data(client,REG_TPS6518X_REVID);
...
#if 0
	/*
	 * support TPS6518X/181 1.2, TPS65185/186 1.0, TPS65185 1.1 and TPS65185 1.2
	 */
	if (!((tps6518x->revId == 0x60) || (tps6518x->revId == 0x61) || (tps6518x->revId == 0x45) || (tps6518x->revId == 0x46) || (tps6518x->revId == 0x55) || (tps6518x->revId == 0x65)))
	{
...
/*2011/05/31 FY11 : Added retry for first i2c communication with TPS65181. */
	if ( tps6518x->revId < 0 )
	{
		...
		if ( retry < 0 )
		{
			dev_err( tps6518x->dev, "read rev id failed. retryout\n" );
			return tps6518x->revId;
		}
		//dev_err( tps6518x->dev, "read rev id failed. retry=%d\n", TPS6518x_1st_I2C_RETRY - retry );
		msleep(50);
		goto i2c_retry;
	}
	else
	{
		...
		if ( tps6518x->revId < 0 )
		{
			dev_err( tps6518x->dev, "final read rev id failed.\n" );
			return tps6518x->revId;
		}
	}
#endif
...
static int tps6518x_i2c_probe(struct i2c_client *client,
			    const struct i2c_device_id *id)
{
	...
	tps6518x->gpio_pmic_pwr2_ctrl = pdata->gpio_pmic_pwr2_ctrl;
	tps6518x->gpio_pmic_pwr0_ctrl = pdata->gpio_pmic_pwr0_ctrl;
/* 2011/05/26 FY11 : Supported TPS65181. */
	tps6518x->gpio_pmic_v3p3_ctrl = pdata->gpio_pmic_v3p3_ctrl;
/* 2011/07/05 FY11 : Added VSYS_EPD_ON. */
	tps6518x->gpio_pmic_vsys = pdata->gpio_pmic_vsys;

	tps6518x->pass_num = pdata->pass_num;
	tps6518x->vcom_uV = pdata->vcom_uV;

	tps6518x->pmic_setup = false;

	ret = platform_driver_register(&tps6518x_regulator_driver);
	if (ret < 0)
		goto err;
...
/* 2011/1/24 FY11 : Added wakeup port control. */
	...
	for (i = 0; i < TPS6518X_NUM_REGULATORS; i++) {
		ret = tps6518x_register_regulator(tps6518x, i, &pdata->regulator_init[i]);
		if (ret != 0) {
			dev_err(tps6518x->dev, "Platform init() failed: %d\n",
			ret);
		goto err;
		}
	}
/* 2011/1/24 FY11 : Added wakeup port control. */
	...
	tps6518x->max_wait = pdata->vpos_pwrup + pdata->vneg_pwrup +
...
static int tps6518x_i2c_remove(struct i2c_client *i2c)
{
...
static const struct i2c_device_id tps6518x_i2c_id[] = {
       { "tps6518x", 0 },
       { }
};
MODULE_DEVICE_TABLE(i2c, tps6518x_i2c_id);
...
static struct i2c_driver tps6518x_i2c_driver = {
	...
	.probe = tps6518x_i2c_probe,
	.remove = tps6518x_i2c_remove,
	.id_table = tps6518x_i2c_id,
};

static int __init tps6518x_init(void)
{
	return i2c_add_driver(&tps6518x_i2c_driver);
}
module_init(tps6518x_init);

static void __exit tps6518x_exit(void)
{
	i2c_del_driver(&tps6518x_i2c_driver);
}
